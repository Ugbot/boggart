...
double lite_get_scale(void) { return get_scale(); }
...
int main(int argc, char **argv) {
  ...
  (void) luaL_dostring(L,
     ...
     * (app/data/, where the binary is built to the repo root next to the
     * boggart CLI). Trying both means `./boggart-studio` works from a fresh
     * checkout with no install step. */
    "  package.path = EXEDIR .. '/data/?.lua;' .. package.path\n"
    "  package.path = EXEDIR .. '/data/?/init.lua;' .. package.path\n"
    "  package.path = EXEDIR .. '/app/data/?.lua;' .. package.path\n"
    "  package.path = EXEDIR .. '/app/data/?/init.lua;' .. package.path\n"
    "  DATADIR = EXEDIR .. '/data'\n"
    "  if not io.open(DATADIR .. '/core/init.lua') then DATADIR = EXEDIR .. '/app/data' end\n"
