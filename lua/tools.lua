...
local function render_tool_file(name, description, input_schema, body)
  return table.concat({
    ...
    "local body = " .. string.format("%q", body),
    'local runner = assert(load("return function(args)\\n" .. body .. "\\nend", "@tool:' .. name .. '"))()',
    "return {",
  ...
  for _, fname in ipairs(sys.listdir(dir) or {}) do
    if fname:match("%.lua$") then
      ...
      local ok, def = pcall(function()
        local chunk = assert(loadfile(dir .. "/" .. fname))
        return chunk()
      end)
      if ok and type(def) == "table" and type(def.run) == "function" then
        M.registry[name] = def
      else
