...
-- boggart-studio appends the agent's model and state on the right, so the
-- thing most likely to be wrong (which model am I about to spend money on?)
-- is visible without opening anything.
local function studio_items()
  local ok, studio = pcall(require, "core.studio")
  if not ok or not studio or not studio.status_items then return nil end
  local okk, items = pcall(studio.status_items)
  return okk and items or nil
end
...
-- Append the agent's model and state to the right-hand items.
function StatusView:with_studio(right)
  local extra = studio_items()
  if not extra or #extra == 0 then return right end
  right[#right + 1] = self.separator
  for _, v in ipairs(extra) do right[#right + 1] = v end
  return right
end
