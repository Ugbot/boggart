...
M.tree = {
  ...
  File = {
    ...
    { "Remove folder\u{2026}", "studio:remove-folder" },
    M.SEP,
    { "Save",           "doc:save" },
  ...
  Edit = {
    ...
    M.SEP,
    { "Next conflict",            "conflicts:next" },
    { "Previous conflict",        "conflicts:previous" },
  ...
  Go = {
    ...
    { "Bookmarks\u{2026}",     "bookmarks:list" },
    { "Search the project\u{2026}", "project-search:find" },
  ...
  Agent = {
    ...
    { "All settings\u{2026}",   "agent:settings-list" },
    ...
    M.SEP,
    { "Search history\u{2026}", "agent:history-search" },
    { "Toggle approvals",     "agent:toggle-approval" },
    { "Approve pending tool", "agent:approve" },
    { "Reject pending tool",  "agent:reject" },
    M.SEP,
  ...
  Run = {
    ...
    M.SEP,
    -- Recipes: saved prompts you run again. These existed and were reachable
    -- only from the toolbar, which is why nobody found them.
    { "Run recipe\u{2026}",        "agent:run-recipe" },
    { "Save as recipe\u{2026}",    "agent:save-recipe" },
    { "Edit recipe\u{2026}",       "agent:edit-recipe" },
    { "Schedule recipe\u{2026}",   "agent:schedule-recipe" },
    { "Stop scheduled recipe",  "agent:stop-schedule" },
    M.SEP,
    { "ReAct until goal\u{2026}",  "agent:react" },
  },
  -- The inbound control plane: the C listener (src/lserve.c) plus the Lua
  ...
  Tools = {
    ...
    { "Inline edit\u{2026}",    "agent:inline-edit" },
