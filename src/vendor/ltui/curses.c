...
#if defined(NCURSES_VERSION)
    setlocale(LC_ALL, "");
    /* boggart: LC_ALL="" adopts the user's LC_NUMERIC, and in comma-decimal
     * locales that makes printf("%.14g") emit "1,5". lua/json.lua formats
     * numbers that way, so every API payload would be silently corrupted.
     * Curses needs LC_CTYPE for wide glyphs, not LC_NUMERIC -- pin it back. */
