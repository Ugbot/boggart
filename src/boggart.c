  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
      printf(
        ...
        "usage:\n"
        "  boggart                    interactive REPL (may spawn a fleet)\n"
        "  boggart --tui              full-screen chat: transcript + agents pane\n"
        ...
        "Every mode runs the one swarm runtime; they differ only in the front end.\n"
        "While a turn's fleet runs: Ctrl-C cancels, p pauses/resumes, k kills the newest.\n\n"
        "options:\n"
